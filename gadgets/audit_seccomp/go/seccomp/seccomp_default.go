...
// Taken from:
// https://github.com/containerd/containerd/blob/00824bf39e98b454310d52cabda6af087f74957f/contrib/seccomp/seccomp_default.go
// And modified to avoid detecting the environment, so it can run in WASM
...
func kernelVersionGreaterEqualThan(minVersion KernelVersion) (bool, error) {
	// FIXME: hardcoded
	kv := KernelVersion{
		Kernel: 6,
		Major:  5,
...
func DefaultProfile(sp *specs.Spec) *specs.LinuxSeccomp {
	...
	if ok, err := kernelVersionGreaterEqualThan(
		KernelVersion{Kernel: 4, Major: 8}); err == nil {
		if ok {
		...
		case "s390", "s390x":
			s.Syscalls = append(s.Syscalls, specs.LinuxSyscall{
				...
				Args: []specs.LinuxSeccompArg{
					{
						Index:    1,
						Value:    CLONE_NEWNS | CLONE_NEWUTS | CLONE_NEWIPC | CLONE_NEWUSER | CLONE_NEWPID | CLONE_NEWNET | CLONE_NEWCGROUP,
						ValueTwo: 0,
