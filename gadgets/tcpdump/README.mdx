...
If you want to let tcpdump analyze the captured traffic directly, you can use the `pcap-ng` output mode and pipe IGs
output directly to tcpdump like so:

```bash
$ ig run --verify-image=false tcpdump:mine --host --pf-main "port 80" -o pcap-ng | tcpdump -nvr -
