	...
	switch(case_id) 
	{
        case 7: // IPv4, IPv6, Port (0b111)
			if(((key.dst.version == 4 && ipaddr_v4_filter == key.dst.addr_raw.v4)
			 || (key.dst.version == 6 && __builtin_memcmp(&ipaddr_v6_filter, key.dst.addr_raw.v6, 16) == 0))
			 &&  port == key.dst.port ){
        ...
        case 3: // No IPv4, IPv6, Port (0b011)
			if(key.dst.version == 6 && __builtin_memcmp(&ipaddr_v6_filter, key.dst.addr_raw.v6, 16) == 0 && port == key.dst.port){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
        case 2: // No IPv4, IPv6, No Port (ob010)
			if(key.dst.version == 6 && __builtin_memcmp(&ipaddr_v6_filter, key.dst.addr_raw.v6, 16) == 0){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
