...
volatile const struct gadget_l3endpoint_t filter_ip = {0};
...
static __always_inline int packet_drop(struct __sk_buff *skb){
	...
	struct event event = {0} ;							/* The sturct to store the information regarding the event */						
	...
	if (filter_ip.version == 4 && filter_ip.addr_raw.v4 ) v4_set = 1;
	if (filter_ip.version == 6) v6_set = 1;
	...
	switch(case_id) 
	{
        case 5: // IPv4, No IPv6, Port (0b101)
			if(key.dst.version == 4 && filter_ip.addr_raw.v4 == key.dst.addr_raw.v4  && port == key.dst.port){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
        case 4: // IPv4, No IPv6, No Port (0b100)
			if(key.dst.version == 4 && filter_ip.addr_raw.v4 == key.dst.addr_raw.v4){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
        case 3: // No IPv4, IPv6, Port (0b011)
			if(key.dst.version == 6 && __builtin_memcmp((const void *)filter_ip.addr_raw.v6, key.dst.addr_raw.v6, 16) == 0 && port == key.dst.port){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
        case 2: // No IPv4, IPv6, No Port (ob010)
			if(key.dst.version == 6 && __builtin_memcmp((const void *)filter_ip.addr_raw.v6, key.dst.addr_raw.v6, 16) == 0){
				return rand_pkt_drop_map_update(&event, &key,&sockets_key_for_md);
			}
            break;
        ...
        case 0: // No IPv4, No IPv6, No Port (0b000)
            ...
            break;
		default:
			// "default case : something wrong with the logic. Also case 6 and 7 are not possible"
			return TC_ACT_OK;
    }
