...
This is a input filter parameter based on which the gadget randomly drops packets to/from this IPv4/IPv6 address. 
It is input as a string in the CLI. By default, its value is 0.0.0.0, which means all the packets to all the IPv4
address will be dropped based on the loss_percentage. Gadget will throw error if we an input invalid IP address.
