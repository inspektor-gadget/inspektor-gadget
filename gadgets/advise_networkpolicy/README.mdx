...
Then, start the advise_networkpolicy gadget in another terminal, redirecting its output to a textfile.

```bash
$ kubectl gadget run advise_seccomp:%IG_TAG% > events.json
```
...
Let's switch back to the gadget terminal, stop our gadget and generate our policy.

```bash
$ kubectl gadget run advise_seccomp:%IG_TAG% > events.json
...
^C
$ kubectl-gadget tool report-network-policy --input events.json
