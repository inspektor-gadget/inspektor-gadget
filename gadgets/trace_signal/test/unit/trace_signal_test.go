// GADGET_TAG=trace-signal-unit-tests IG_VERIFY_IMAGE=false go test -v -exec 'sudo -E' ./gadgets/trace_signal/test/unit/…
...
func TestTraceSignalGadget(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			fmt.Printf("printing %d events\n", len(gadgetRunner.CapturedEvents))
			for _, event := range gadgetRunner.CapturedEvents {
				fmt.Printf("signal: %s, sig_raw: %d, error: %s\n", event.Signal, event.SignalRaw, event.Error)
			}
