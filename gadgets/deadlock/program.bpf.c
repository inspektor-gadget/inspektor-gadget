 ...
 */
static __always_inline int trace_mutex_acquire(struct pt_regs *ctx,
					       u64 mutex_addr)
{
	...
	u32 pid = pid_tgid >> 32;
	u64 mutex = mutex_addr;

	struct held_mutex empty_value
		[MAX_HELD_MUTEXES] = {}; // represents an empty list of held mutexes
	struct held_mutex *held_mutexes = bpf_map_lookup_or_try_init(
		&thread_to_held_mutexes, &tid, &empty_value);
	if (!held_mutexes) {
		bpf_printk(
 ...
 */
static __always_inline int trace_mutex_release(struct pt_regs *ctx,
					       u64 mutex_addr)
{
	u64 mtns_id = gadget_get_mntns_id();
	if (gadget_should_discard_mntns_id(mtns_id))
