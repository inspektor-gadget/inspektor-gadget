...
`
	...
	scriptsCmd := "echo '#!/bin/sh\nls' > /bin/with_shabeng.sh ; /dev/script.sh echo ls > /dev/script.sh ; echo ls > /bin/script.sh ; chmod +x /dev/script.sh /bin/script.sh /bin/with_shabeng.sh ; /dev/script.sh ; /bin/script.sh ; /bin/with_shabeng.sh;"
	...
	cmd := fmt.Sprintf("%s %s && cp /usr/bin/sh /usr/bin/sh2 && nsenter --setuid 1000 --setgid 1111 /usr/bin/sh2 -c '%s'", scriptsCmd, buildCmd, innerCmd)
		...
		igrunner.WithValidateOutput(
			func(t *testing.T, output string) {
				expectedEntries := []*traceExecEvent{
					...
					{
						CommonData:  utils.BuildCommonData(containerName, commonDataOpts...),
						Proc:        utils.BuildProc("with_shabeng.sh", 0, 0),
						Cwd:         "/",
						Args:        "/bin/with_shabeng.sh",
						PupperLayer: false,
						UpperLayer:  false,
						FupperLayer: true,
						Exepath:     "/usr/bin/dash",
						File:        "/usr/bin/with_shabeng.sh",
						Error:       "",
						DevMajor:    utils.NormalizedInt,
						DevMinor:    utils.NormalizedInt,
						Inode:       utils.NormalizedInt,
