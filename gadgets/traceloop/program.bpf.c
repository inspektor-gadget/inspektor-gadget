...
int ig_traceloop_e(struct bpf_raw_tracepoint_args *ctx)
{
	...
	if (!args) {
		bpf_error_printk(
			"enter: there should be a pt_regs for key %lu", pid);
...
#pragma unroll
	for (i = 0; i < SYSCALL_ARGS; i++) {
		...
		struct syscall_event_cont_t sc_cont = { .event_type = SYSCALL_EVENT_CONT };
...
int ig_traceloop_x(struct bpf_raw_tracepoint_args *ctx)
{
	...
	if (!args) {
		bpf_error_printk(
			"exit: there should be a pt_regs for key %lu", pid);
