...
struct Tracelooper {
    ...
    readers: Mutex<HashMap<u64, PerfReader>>,
}

impl Default for Tracelooper {
    fn default() -> Self {
...
impl Tracelooper {
    fn attach(&self, mount_ns_id: u64) -> Result<(), String> {
        ...
        self.readers
            .lock()
            .unwrap()
            .insert(mount_ns_id, perf_reader);
    ...
    fn read(&self, mount_ns_id: u64, perf_reader: &PerfReader) -> Result<Vec<Event>, String> {
            ...
            match sys_event.event_type {
                EventType::Enter | EventType::Exit => {
                    let mut event = SyscallEvent {
                        ...
                        mount_ns_id,
                        args: Vec::with_capacity(SYSCALL_ARGS),
                        retval: 0,
                    };
                    ...
                    match event.typ {
                        EventType::Enter => {
                            for i in 0..SYSCALL_ARGS {
                                event.args.push(sys_event.args[i]);
                            }
                    ...
                    match target.entry(event.monotonic_ts) {
                        Entry::Vacant(e) => {
                            e.insert(vec![event]);
                        }
                ...
                EventType::Cont => {
                    ...
                    if sys_event_cont.failed != 0 {
                        ...
                        event.param = api::helpers::from_c_string_n(
                            &sys_event_cont.param,
                            sys_event_cont.length as usize,
                        );
                    }

                    event.param = escape_string::escape(&event.param).to_string();

                    match syscall_continued_events_map.entry(event.monotonic_ts) {
                        Entry::Vacant(e) => {
                            e.insert(vec![event]);
                        }
                        Entry::Occupied(mut e) => {
        ...
        for (enter_ts, enter_ts_events) in &syscall_enter_events_map {
            for enter_event in enter_ts_events {
                ...
                let syscall_declaration = match syscall_declaration_cache.get(&event.syscall) {
                    Ok(decl) => decl,
                    Err(err) => return Err(format!("getting syscall definition: {}", err)),
                };

                let parameters_number = syscall_declaration.params.len();
                event.parameters.reserve(parameters_number);
                ...
                for i in 0..parameters_number {
                    ...
                    if syscall_continued_events_map.contains_key(enter_ts) {
                        for syscall_cont_event in &syscall_continued_events_map[enter_ts] {
                            if usize::from(syscall_cont_event.index) != i {
                                continue;
                            }

                            syscall_param.content = syscall_cont_event.param.clone();
        ...
        for enter_ts in &to_delete {
            syscall_enter_events_map.remove(enter_ts);
        }
...
static TRACELOOPER: LazyLock<Mutex<Tracelooper>> =
    LazyLock::new(|| Mutex::new(Tracelooper::default()));
...
fn params_to_string(parameters: Vec<SyscallParam>) -> String {
    ...
    for (idx, p) in parameters.iter().enumerate() {
        ...
        ret.push_str(&format!("{}={}", p.name, value));
...
fn gadgetInit() -> i32 {
    ...
    for field_info in fields_info {
        ...
        match DS_OUTPUT.lock().unwrap().add_field(name, field_info.kind) {
...
fn gadgetStart() -> i32 {
    ...
    let syscall_filters: Vec<_> = if !syscall_filter_param.is_empty() {
...
fn gadgetStop() -> i32 {
    let tracelooper = TRACELOOPER.lock().unwrap();
    let readers = tracelooper.readers.lock().unwrap();
    for (mount_ns_id, reader) in readers.iter() {
        ...
        for event in events {
            let ds_output = DS_OUTPUT.lock().unwrap();
