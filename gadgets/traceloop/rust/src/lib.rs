        ...
        loop {
            ...
            match sys_event.event_type {
                ...
                EventType::Cont => {
                    ...
                    let param = if sys_event_cont.failed != 0 {
                        "(Failed to dereference pointer)".to_string()
                    } else if sys_event_cont.length == USE_NULL_BYTE_LENGTH {
                        api::helpers::from_c_string(&sys_event_cont.param)
                    } else {
                        api::helpers::from_c_string_n(
                            &sys_event_cont.param,
                            sys_event_cont.length as usize,
                        )
                    };

                    let event = SyscallEventContinued {
                        monotonic_ts: sys_event_cont.monotonic_ts,
                        index: sys_event_cont.index,
                        param: escape_string::escape(&param).to_string(),
                    };
