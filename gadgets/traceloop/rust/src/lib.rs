...
#![feature(ip_from)]
use api::{
    datasources::{Data, DataSource, DataSourceType, Field, FieldKind, Packet},
    fields::FieldData,
...
static SYSCALL_DEFS: LazyLock<HashMap<&'static str, [u64; 6]>> = LazyLock::new(|| {
    ...
    m.insert(
        ...
        [
            0,
            USE_ARG_INDEX_AS_PARAM_LENGTH + 2,
            0,
            0,
            0,
            0,
...
impl Tracelooper {
    fn attach(&mut self, mount_ns_id: u64) -> Result<(), String> {
        ...
        if let Err(err) = self.map_of_perf_buffers.update(
            ...
            return Err(format!(
                "adding perf buffer to map with mount_ns_id {}: {}",
                mount_ns_id, err
            ));
        }
    ...
    fn detach(&self, mount_ns_id: u64) -> Result<(), String> {
        match self.map_of_perf_buffers.delete(&mount_ns_id) {
            Ok(()) => Ok(()),
            Err(err) => Err(format!(
                "removing perf buffer from map with mount_ns_id {}: {}",
                mount_ns_id, err
            )),
        }
    }
    ...
    fn read(&self, mount_ns_id: u64, perf_reader: &PerfReader) -> Result<Vec<Event>, String> {
        ...
        loop {
            ...
            match sys_event.event_type {
                ...
                EventType::Cont => {
                    ...
                    let mut event = SyscallEventContinued {
                    ...
                    if sys_event_cont.failed != 0 {
                        event.param = "(Failed to dereference pointer)".to_string();
                    } else if sys_event_cont.length == USE_NULL_BYTE_LENGTH {
                        event.param =                         api::helpers::from_c_string(&sys_event_cont.param);
                        event.param_raw_length = event.param.len();
                    } else {
                        if sys_event_cont.length < sys_event_cont.param.len() as u64 {
                            event.param =                             String::from_utf8_lossy(&sys_event_cont.param[..sys_event_cont.length as usize]).into_owned();
                            event.param_raw_length = sys_event_cont.length as usize;
                        } else {
                            event.param = String::from_utf8_lossy(&sys_event_cont.param).into_owned();
                        }
                    };

                    event.param = format!("\"{}\"", event.param.escape_default());

                    syscall_continued_events_map.entry(event.monotonic_ts).or_default().push(event);
                }
            };
        }
        ...
        for (enter_ts, enter_ts_events) in &syscall_enter_events_map {
            for enter_event in enter_ts_events {
                ...
                for i in 0..parameters_number {
                    ...
                    if let Some(syscall_cont_events) = syscall_continued_events_map.get(enter_ts) {
                        for syscall_cont_event in syscall_cont_events {
                            if usize::from(syscall_cont_event.index) != i {
                                continue;
                            }

                            syscall_param.content = if event.syscall == "connect" && param_name == "uservaddr" {
                                match sockaddr_from_bytes(&syscall_cont_event.param_raw[..syscall_cont_event.param_raw_length]) {
                                    Ok(addr) => addr,
                                    Err(err) => return Err(err),
                                }
                            } else {
                                syscall_cont_event.param.clone()
                            };
...
fn params_to_string(parameters: &[SyscallParam]) -> String {
...
fn sockaddr_from_bytes(data: &[u8]) -> Result<String, String> {
    ...
    if size < SOCK_ADDR_MIN_SIZE {
        return Err(format!("sockaddr byte slice too short to determine family, expected at least {}, got: {}", SOCK_ADDR_MIN_SIZE, size));
    }
    ...
    match family {
        AF_INET => {
            if size < SOCK_ADDR_IN_MIN_SIZE {
                return Err(format!("IPv4 sockaddr byte slice too short, expected {}, got : {}", SOCK_ADDR_IN_MIN_SIZE, size));
            }
        ...
        AF_INET6 => {
            if size < SOCK_ADDR_IN_6_MIN_SIZE {
                return Err(format!("IPv6 sockaddr byte slice too short, expected {}, got: {}", SOCK_ADDR_IN_6_MIN_SIZE, size));
            }
        ...
        AF_UNIX => {
            if size < SOCK_ADDR_UN_MIN_SIZE {
                return Err(format!("Unix sockaddr byte slice too short, expected {}, got: {}", SOCK_ADDR_UN_MIN_SIZE, size));
            }
            ...
            Ok(if path_bytes[0] == 0 {
                ...
                format!("unix:@{}", String::from_utf8_lossy(&path_bytes[1..]).into_owned())
            } else {
                if let Some(null_terminator) = path_bytes.iter().position(|&b| b == 0) {
                    // If a null terminator is found, the path is up to that point.
                    format!("unix:{}", String::from_utf8_lossy(&path_bytes[..null_terminator]).into_owned())
                } else {
                    ...
                    format!("unix:{}", String::from_utf8_lossy(&path_bytes).into_owned())
                }
            })
        }
...
fn gadgetStart() -> i32 {
    ...
    let syscall_filters = match syscall_filter_param.is_empty() {
        true => vec![],
        false => syscall_filter_param.split(",").collect()
    };
    ...
    let ds = match DataSource::get_datasource(api::datasources::DATA_SOURCE_CONTAINERS.to_string())
    {
        Ok(ds) => ds,
        Err(err) => {
            api::errorf!(
                "finding datasource {}: {:?}",
                api::datasources::DATA_SOURCE_CONTAINERS,
                err
            );
            return 1;
        }
    ...
    let err = ds.subscribe(
        move |_source: DataSource, data: Data| {
            let event_type = match event_type_field.string(
                data,
                api::datasources::DATA_SOURCE_CONTAINERS_EVENT_TYPE_MAX_SIZE as u32,
            ) {
    ...
    for (mount_ns_id, reader) in &tracelooper.readers {
        ...
        for event in events {
            ...
            if let Err(err) = fields
                .parameters
                .set_data(packet_data, &params_to_string(&event.parameters))
            {
