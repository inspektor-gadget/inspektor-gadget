...
// It handles IPv4 (AF_INET), IPv6 (AF_INET6) and Unix (AF_UNIX) address families.
func sockaddrFromBytes(data []byte) (string, error) {
	...
	case AF_UNIX:
		...
		// The sun_path field starts at offset 2 (after sun_family).
		// It's a fixed-size array, typically 108 bytes, and is null-terminated
		// for filesystem paths. For abstract sockets, the first byte of sun_path is 0.
		pathBytesRaw := data[2:110]

		// Find the null terminator.
		nullTerminatorIdx := len(pathBytesRaw)
		for i, b := range pathBytesRaw {
			if b == 0 {
				nullTerminatorIdx = i
				break
			}
		...
		// Extract the actual path bytes up to the null terminator
		pathBytes := pathBytesRaw[:nullTerminatorIdx]

		path := string(pathBytes)
		if len(path) > 0 && path[0] == 0 { // Abstract socket (Linux-specific)
			// For abstract sockets, the first byte is \x00, and the name starts after it.
			return fmt.Sprintf("unix@%s", path[1:]), nil
		}
