...
func wallTimeFromBootTime(ts uint64) int64 {
	if ts == 0 {
		return time.Now().UnixNano()
	}
	return time.Unix(0, int64(ts)).Add(0/*timeDiff*/).UnixNano()
}
...
func (t *tracelooper) read(reader *containerRingReader) ([]*event, error) {
...
func gadgetStop() int32 {
	t.readers.Range(func(_, value any) bool {
		...
		for _, event := range events {
			...
			fields["mntns_id"].SetUint64(api.Data(packet), event.mountNsID)
