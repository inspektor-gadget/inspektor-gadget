...
type ExpectedTraceInitEvent struct {
...
type testDef struct {
	...
	validateEvent  func(t *testing.T, info *utils.RunnerInfo, expectedLen uint64, expectedParams string, events []ExpectedTraceInitEvent)
}

func TestTraceInitGadget(t *testing.T) {
	...
	cases := map[string]testDef{
		"captures_all_events_with_no_filters_configured": {
			...
			validateEvent: func(t *testing.T, info *utils.RunnerInfo, expectedLen uint64, expectedParams string, events []ExpectedTraceInitEvent) {
				utils.ExpectAtLeastOneEvent(func(info *utils.RunnerInfo, _ uint64) *ExpectedTraceInitEvent {
					proc := info.Proc
					utils.NormalizeParentTid(&proc)
					return &ExpectedTraceInitEvent{
						Proc:        proc,
						Len:         expectedLen,
		...
		"captures_no_events_with_no_matching_filter": {
			...
			generateEvent: generateInitModuleEvent,
			validateEvent: func(t *testing.T, info *utils.RunnerInfo, _ uint64, _ string, events []ExpectedTraceInitEvent) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			normalizeEvent := func(event *ExpectedTraceInitEvent) {
				utils.NormalizeParentTid(&event.Proc)
			}

			opts := gadgetrunner.GadgetRunnerOpts[ExpectedTraceInitEvent]{
				Image:          "trace_init",
