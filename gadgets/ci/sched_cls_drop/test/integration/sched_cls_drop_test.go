...
func TestSchedCLS(t *testing.T) {
	...
	var nsTest string
	serverContainerOpts := []containers.ContainerOption{containers.WithContainerImage(gadgettesting.BusyBoxImage)}
	clientContainerOpts := []containers.ContainerOption{containers.WithContainerImage(gadgettesting.BusyBoxImage)}
	...
	time.Sleep(10 * time.Second) // Wait for the server to be ready

	serverIP := serverContainer.IP()

	clientContainer := containerFactory.NewContainer(
		clientContainerName,
		fmt.Sprintf("ping %s", serverIP),
		clientContainerOpts...,
	)
