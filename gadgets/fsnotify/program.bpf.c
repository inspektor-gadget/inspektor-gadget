...
{
	// inotify_handle_inode_event is introduced in Linux v5.10:
...
int BPF_KPROBE(inotify_handle_event_e, struct fsnotify_group *group, u32 mask,
	       ...
	       const struct qstr *file_name)
{
	if (!fanotify_only) {
		...
		struct fsnotify_insert_event_value value = {
			.type = inotify,
			.i_ino = 0, // FIXME: not available on Linux < 5.11
			.i_ino_dir = BPF_CORE_READ(dir, i_ino),
		};
		// context for fsnotify_insert_event
		bpf_map_update_elem(&fsnotify_insert_event_ctx, &pid_tgid,
