...
func TestMyGadget(t *testing.T) {
  ...
  mygadgetCmd := igrunner.New(
    // gadget repository and tag are added with the following environment variables:
    // - $GADGET_REPOSITORY
    // - $GADGET_TAG
    "mygadget",
    igrunner.WithFlags("--runtimes=docker"),
    // used to start the gadget, then, wait for other commands to run (cat /dev/null in this case)
...
Before running the test we need to tag the image (skip if already tagged) and define environment variables for gadget repository and tag.

```bash
$ export GADGET_REPOSITORY=ghcr.io/my-org GADGET_TAG=latest
$ sudo -E ig image tag mygadget:latest $GADGET_REPOSITORY/mygadget:$GADGET_TAG
```
