...
`namespace`, `podname`, `containername` and `selector` using this mechanism. You
can still use the `--filter` flag to filter by those fields in [user
space](#filtering-in-user-space).
