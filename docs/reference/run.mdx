...
use complex expressions based on the [Expr language](https://expr-lang.org/).
...
```bash
--filter-expr 'proc.comm in ["wget", "curl"] and dst.addr in cidr("1.0.0.0/8") and runtime.containerImageName matches "ybox$"'
```

The command has to be either wget or curl. The source address has to be in the specified CIDR range. And the container image name has to end with `ybox`.
