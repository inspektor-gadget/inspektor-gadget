 ...
 * `-c string`, `--containername string`, show only data from containers with the runtime-assigned name (not the name defined in the pod spec)
 * `--k8s-containername string`, show only data from containers with the name defined in the pod spec
...
* `--namespace string`, show data from pods in that namespace.
* `--podname string`, show only data from pods with that name.
* `-l string`, `--selector string`: show only data that matches the given
label or selector. Only `=` is currently supported (e.g. `key1=value1,key2=value2`).
