...
On arm64, the Python unwinder may fail to resolve interpreted frames with
certain Python versions (e.g., Python 3.13). This is due to the arm64
instruction decoder not handling the code patterns generated by newer Python
builds. See [upstream issue #251](https://github.com/open-telemetry/opentelemetry-ebpf-profiler/issues/251)
for context. The x86-64 decoder was updated to handle these patterns, but
the arm64 decoder has not yet been updated.
