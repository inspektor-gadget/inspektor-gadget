...
Operators are the cornerstone of the Inspetkor Gadget architecture. They are
...
![Operators Pipeline](./operators-pipeline.svg)

Operators are enabled automatically based on the gadget that's being run,
according to the layers present on the OCI image, to the types used by the eBPF
...
plan](https://github.com/inspektor-gadget/inspektor-gadget/issues/2497) to
support third-party operators in the future.
...
Parameters are passed as flags when [running](../../reference/run.mdx) a gadget using the CLI
tools. When using the [APIs](../../apis), it's required to use the full
qualified name.
