...
func (r *Runtime) GetOCIGadgetInfo(gadgetCtx runtime.GadgetContext, params *params.Params, args []string) (*api.GadgetInfo, error) {
	fmt.Printf("GetOCIGadgetInfo: params are %+v\n", gadgetCtx.Params())

	ociParams := gadgetCtx.OperatorsParamCollection()["oci"]
	ocihandler, err := operators.NewOCIHandler(gadgetCtx, ociParams, gadgetCtx.Args())
	if err != nil {
		return nil, fmt.Errorf("instantiating: %w", err)
	}

	err = ocihandler.PrepareOps()
	if err != nil {
