...
func loadSigningInformation(ctx context.Context, imageRef reference.Named, imageStore oras.Target, repo *remote.Repository) ([]byte, []byte, error) {
	...
	signature, payload, loadSignatureTagErr := _loadSigningInformation(ctx, imageRef, imageStore, repo, signatureTag, false)
	...
	signature, payload, loadIndexTagErr := _loadSigningInformation(ctx, imageRef, imageStore, repo, indexTag, true)
