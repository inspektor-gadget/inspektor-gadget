...
func (s *Symbolizer) resolveStackItemsWithTable(table *symbolTable, baseAddress uint64, stackItems []StackItemQuery, res []StackItemResponse) {
	table.timestamp = time.Now()
	for idx := range stackItems {
		symbol := table.lookupByAddr(stackItems[idx].Addr - baseAddress)
		if symbol != "" {
			res[idx].Found = true
