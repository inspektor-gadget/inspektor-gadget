	...
	for i, query := range stackItems {
		...
		if err != nil {
			if os.IsNotExist(err) {
				suggestedCmd := fmt.Sprintf("sudo DEBUGINFOD_URLS=https://debuginfod.elfutils.org debuginfod-find debuginfo %s", buildIDStr)
		...
		if fi, err := file.Stat(); err != nil {
			...
			suggestedCmd := fmt.Sprintf("sudo rm -f %s", debuginfoPath)
