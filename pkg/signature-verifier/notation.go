...
func getSignatures(ctx context.Context, repo *remote.Repository, imageStore oras.Target, ref reference.Named) ([][]byte, error) {
	...
	err = repo.Referrers(ctx, desc, registry.ArtifactTypeNotation, func(references []ocispec.Descriptor) error {
		for _, reference := range references {
			...
			for _, layer := range signatureManifest.Layers {
				// JWS are not supported.
				if layer.MediaType != cose.MediaTypeEnvelope {
					continue
				}
...
func getEnvelope(signatureBytes []byte) (*signature.EnvelopeContent, error) {
	envelope, err := signature.ParseEnvelope(cose.MediaTypeEnvelope, signatureBytes)
	if err != nil {
...
func (n *notationVerifier) Verify(ctx context.Context, repo *remote.Repository, imageStore oras.Target, ref reference.Named) error {
	...
	signaturesBytes, err := getSignatures(ctx, repo, imageStore, ref)
	if err != nil {
		return fmt.Errorf("getting signature bytes: %w", err)
	}
	...
	for _, signatureBytes := range signaturesBytes {
		content, err := getEnvelope(signatureBytes)
		if err != nil {
			return fmt.Errorf("getting envelope: %w", err)
		}

		_, err = signature.VerifyAuthenticity(&content.SignerInfo, certificates)
		if err != nil {
			return fmt.Errorf("veryfing authenticity: %w", err)
		}
	}
