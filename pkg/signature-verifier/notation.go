...
func validateTrustStorePart(trustStorePart string) error {
	if strings.Count(trustStorePart, "/") > 0 {
		return fmt.Errorf("trust store name must not contain \"/\": %q", trustStorePart)
	}

	if trustStorePart == "." || trustStorePart == ".." {
		return fmt.Errorf("trust store name must not be \".\" or \"..\": %q", trustStorePart)
	}

	return nil
}

func getAndValidateTrustStore(rawTrustStore string) (string, error) {
	...
	for _, part := range parts {
		if err := validateTrustStorePart(part); err != nil {
			return "", err
		}
	}
...
func addCertificatesToTrustStore(trustStore string, certificates []string) error {
	path := filepath.Join(filestore.GadgetBaseDir, "truststore", "x509", trustStore)
...
func (n *notationVerifier) Verify(ctx context.Context, repo *remote.Repository, imageStore oras.Target, ref reference.Named) error {
	...
	trustStore, err := getAndValidateTrustStore(policy.TrustPolicies[0].TrustStores[0])
	if err != nil {
		return fmt.Errorf("getting and validating trust store: %w", err)
	}

	err = addCertificatesToTrustStore(trustStore, n.certificates)
	if err != nil {
		return fmt.Errorf("adding certificates to trust store: %w", err)
	}

	dir.UserConfigDir = filestore.GadgetBaseDir
