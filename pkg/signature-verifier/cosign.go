...
func _loadSigningInformation(ctx context.Context, imageRef reference.Named, imageStore oras.Target, repo *remote.Repository, signingInfoTag string, useOCI11 bool) ([]byte, []byte, error) {
...
func loadSigningInformation(ctx context.Context, imageRef reference.Named, imageStore oras.Target, repo *remote.Repository) ([]byte, []byte, error) {
	...
	signature, payload, err := _loadSigningInformation(ctx, imageRef, imageStore, repo, signatureTag, false)
	if err == nil {
		return signature, payload, nil
	}
	...
	return _loadSigningInformation(ctx, imageRef, imageStore, repo, indexTag, true)
}
