...
var (
	patternArgumentRegister = regexp.MustCompile("^%([a-z0-9]+)$")
...
func encodeArgument(arg string) (uint64, error) {
	...
	if matches := patternArgumentRegister.FindStringSubmatch(parts[1]); matches != nil {
		// register argument
		encodeField(&encodedValue, 4, 1, 0)

		regName := matches[1]
		if encodedRegister, exist := registerEncoding[regName]; exist {
			encodeField(&encodedValue, 8, 8, encodedRegister)
		} else {
			return 0, fmt.Errorf("unsupported register: %q", regName)
		}
