...
const (
	GadgetCatalogManager_GetCatalog_FullMethodName = "/api.GadgetCatalogManager/GetCatalog"
)

// GadgetCatalogManagerClient is the client API for GadgetCatalogManager service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type GadgetCatalogManagerClient interface {
	GetCatalog(ctx context.Context, in *GetCatalogRequest, opts ...grpc.CallOption) (*GetCatalogResponse, error)
}

type gadgetCatalogManagerClient struct {
	cc grpc.ClientConnInterface
}

func NewGadgetCatalogManagerClient(cc grpc.ClientConnInterface) GadgetCatalogManagerClient {
	return &gadgetCatalogManagerClient{cc}
}

func (c *gadgetCatalogManagerClient) GetCatalog(ctx context.Context, in *GetCatalogRequest, opts ...grpc.CallOption) (*GetCatalogResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GetCatalogResponse)
	err := c.cc.Invoke(ctx, GadgetCatalogManager_GetCatalog_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// GadgetCatalogManagerServer is the server API for GadgetCatalogManager service.
// All implementations must embed UnimplementedGadgetCatalogManagerServer
// for forward compatibility.
type GadgetCatalogManagerServer interface {
	GetCatalog(context.Context, *GetCatalogRequest) (*GetCatalogResponse, error)
	mustEmbedUnimplementedGadgetCatalogManagerServer()
}

// UnimplementedGadgetCatalogManagerServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedGadgetCatalogManagerServer struct{}

func (UnimplementedGadgetCatalogManagerServer) GetCatalog(context.Context, *GetCatalogRequest) (*GetCatalogResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetCatalog not implemented")
}
func (UnimplementedGadgetCatalogManagerServer) mustEmbedUnimplementedGadgetCatalogManagerServer() {}
func (UnimplementedGadgetCatalogManagerServer) testEmbeddedByValue()                              {}

// UnsafeGadgetCatalogManagerServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to GadgetCatalogManagerServer will
// result in compilation errors.
type UnsafeGadgetCatalogManagerServer interface {
	mustEmbedUnimplementedGadgetCatalogManagerServer()
}

func RegisterGadgetCatalogManagerServer(s grpc.ServiceRegistrar, srv GadgetCatalogManagerServer) {
	// If the following call pancis, it indicates UnimplementedGadgetCatalogManagerServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&GadgetCatalogManager_ServiceDesc, srv)
}

func _GadgetCatalogManager_GetCatalog_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetCatalogRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(GadgetCatalogManagerServer).GetCatalog(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: GadgetCatalogManager_GetCatalog_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(GadgetCatalogManagerServer).GetCatalog(ctx, req.(*GetCatalogRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// GadgetCatalogManager_ServiceDesc is the grpc.ServiceDesc for GadgetCatalogManager service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var GadgetCatalogManager_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "api.GadgetCatalogManager",
	HandlerType: (*GadgetCatalogManagerServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "GetCatalog",
			Handler:    _GadgetCatalogManager_GetCatalog_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "api/api.proto",
