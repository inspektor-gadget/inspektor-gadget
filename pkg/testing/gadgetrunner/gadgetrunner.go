...
// Parameters of the oci operator, which are not exported by it.
const (
	ociOperatorName            = "oci"
	ociAuthfileParam           = "authfile"
	ociInsecureRegistriesParam = "insecure-registries"
	ociDisallowPullingParam    = "disallow-pulling"
	ociPullParam               = "pull"
)
...
func (g *GadgetRunner[T]) RunGadget() {
	...
	// Make sure the gadget image is in the local store before starting the
	// workload. RunGadget() pulls the image itself, but a pull from a remote
	// registry can take longer than the workload generated by
	// beforeGadgetRun() lives, in which case the gadget starts with nothing
	// left to observe.
	g.ensureImage(ociParams)
...
// ensureImage pulls the gadget image into the local store if needed. It uses
// the same parameters and the same pull policy as the oci operator, so it
// takes the decision the operator would take later: with the default "missing"
// policy the operator then finds the image in the store and does not touch the
// network. With "always" it pulls again, and the race this is meant to avoid
// remains.
//
// "missing" only checks for presence, never for freshness, here exactly as in
// the operator.
func (g *GadgetRunner[T]) ensureImage(ociParams *params.Params) {
	require.NotNil(g.testCtx, ociParams, "oci operator params not found")

	imgOpts := &oci.ImageOptions{
		AuthOptions: oci.AuthOptions{
			AuthFile:           ociParams.Get(ociAuthfileParam).AsString(),
			InsecureRegistries: ociParams.Get(ociInsecureRegistriesParam).AsStringSlice(),
			DisallowPulling:    ociParams.Get(ociDisallowPullingParam).AsBool(),
		},
		Logger: logger.DefaultLogger(),
	}

	pullPolicy := oci.PullImageMissing
	if v, ok := g.paramValues["operator."+ociOperatorName+"."+ociPullParam]; ok && v != "" {
		pullPolicy = v
	}

	err := oci.EnsureImage(context.Background(), g.image, imgOpts, pullPolicy)
	require.NoError(g.testCtx, err, "ensuring gadget image")
}
