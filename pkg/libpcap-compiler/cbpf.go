...
func (l *libpcapDynamic) init() error {
	l.once.Do(func() {
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load libpcap: %v", err)
			return
		}
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load pcap_open_dead: %v", err)
			return
		}
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load pcap_close: %v", err)
			return
		}
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load pcap_compile: %v", err)
			return
		}
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load pcap_freecode: %v", err)
			return
		}
		...
		if err != nil {
			l.err = fmt.Errorf("failed to load pcap_geterr: %v", err)
			return
		}
...
func CompileCbpf(expr string, l2 bool) (insts []bpf.Instruction, err error) {
	...
	if err = libpcap.init(); err != nil {
		return nil, fmt.Errorf("failed to initialize libpcap: %v", err)
	}
	...
	if libpcap.pcapCompile(pcap, unsafe.Pointer(&bpfProg), cexpr, 1, PCAP_NETMASK_UNKNOWN) < 0 {
		...
		return nil, fmt.Errorf("failed to pcap_compile '%s': %+v", expr, errStr)
	}
