...
// Get returns a map containing processes using eBPF programs.
...
func (p *ProcessMap) Get() (map[uint32][]Process, error) {
	if p.useFallbackIterator {
		return getPidMapFromProcFs()
	}

	return p.bpfIter.Get()
}
