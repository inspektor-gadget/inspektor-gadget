...
func TestGetUnderlyingType(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// TODO: is this right? Shouldn't it be "typedef", "int32"?
			expectedNames: []string{"typedef"},
		},
		{
			...
			expectedType: int32Type,
			// TODO: is this right? Shouldn't it be "typedef", "typedef", "int32"?
			expectedNames: []string{"typedef", "typedef"},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			retTyp, retNames, _ := GetUnderlyingType(tt.typDef)
			assert.Equal(t, tt.expectedType, retTyp)
			assert.Equal(t, tt.expectedNames, retNames)
		})
	}
