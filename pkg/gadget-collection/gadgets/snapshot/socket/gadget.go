...
func (t *Trace) Collect(trace *gadgetv1alpha1.Trace) {
	...
	for _, container := range filteredContainers {
		key := container.K8s.Namespace + "/" + container.K8s.Pod
		if _, ok := visitedPods[key]; !ok {
			...
			log.Debugf("Gadget %s: Using PID %d to retrieve network namespace of Pod %q in Namespace %q",
				trace.Spec.Gadget, container.Pid, container.K8s.Pod, container.K8s.Namespace)

			podSockets, err := socketTracer.RunCollector(container.Pid, container.K8s.Pod,
