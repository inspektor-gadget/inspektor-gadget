...
func (f *TraceFactory) Description() string {
	return `byob runs eBPF program in a OCI registry.`
}
...
func (t *Trace) Start(trace *gadgetv1alpha1.Trace) {
	...
	eventCallback := func(event *byobTypes.Event) {
	...
	if err != nil {
		trace.Status.OperationError = fmt.Sprintf("Failed to start byob tracer: %s", err)
		return
	}
	t.started = true
