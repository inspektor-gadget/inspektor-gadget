...
func (t *Trace) Start(trace *gadgetv1alpha1.Trace) {
	...
	detachContainerFunc := func(container *containercollection.Container) {
		...
		_, ok := t.containerIDs[container.Runtime.ID]
		if ok {
			t.containerIDs[container.Runtime.ID].detached = true
		} else {
			log.Errorf("trace does not know about container with ID %q", container.Runtime.ID)
