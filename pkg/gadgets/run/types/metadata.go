...
const (
	...
	// Prefix used to mark profiler structs
	profilersPrefix = "gadget_profiler_"
...
func validateProfilerMap(traceMap *ebpf.MapSpec, _ string) error {
	if traceMap.Type != ebpf.Hash {
		return fmt.Errorf("map %q has a wrong type, expected: hash, got: %s",
...
func (m *GadgetMetadata) Populate(spec *ebpf.CollectionSpec) error {
	...
	if err := m.populateProfilers(spec); err != nil {
		return fmt.Errorf("handling profilers: %w", err)
	}
...
func (m *GadgetMetadata) populateProfilers(spec *ebpf.CollectionSpec) error {
	profilerInfo, err := getProfilerInfo(spec)
	if err != nil {
		return err
	}
	if profilerInfo == nil {
		log.Debug("No profiler found in eBPF object")
		return nil
	}
	...
	profilerMap := spec.Maps[profilerInfo.mapName]
	if profilerMap == nil {
		return fmt.Errorf("map %q not found in eBPF object", profilerInfo.mapName)
	}

	if err := validateProfilerMap(profilerMap, ""); err != nil {
		return fmt.Errorf("trace map is invalid: %w", err)
	}

	var profilerKeyStruct *btf.Struct
	if err := spec.Types.TypeByName(profilerInfo.keyType, &profilerKeyStruct); err != nil {
		return fmt.Errorf("finding key struct %q in eBPF object: %w", profilerInfo.keyType, err)
	}

	var profilerValueStruct *btf.Struct
	if err := spec.Types.TypeByName(profilerInfo.valueType, &profilerValueStruct); err != nil {
		return fmt.Errorf("finding value struct %q in eBPF object: %w", profilerInfo.valueType, err)
	}

	if _, found := m.Profilers[profilerInfo.name]; !found {
		log.Debugf("Adding profiler %q", profilerMap.Name)
		m.Profilers[profilerMap.Name] = Profiler{
			MapName:         profilerMap.Name,
			StructKeyName:   profilerKeyStruct.Name,
			StructValueName: profilerValueStruct.Name,
		}
	} else {
		log.Debugf("Profiler %q already defined, skipping", profilerInfo.name)
	}

	if err := m.populateStruct(profilerKeyStruct); err != nil {
		return fmt.Errorf("populating key struct: %w", err)
	}

	if err := m.populateStruct(profilerValueStruct); err != nil {
		return fmt.Errorf("populating value struct: %w", err)
	}
...
// getProfilerInfo returns the tracer info generated with GADGET_PROFILER().
// If there are multiple annotations only the first one is returned.
func getProfilerInfo(spec *ebpf.CollectionSpec) (*profilerInfo, error) {
	profilersInfo, err := GetGadgetIdentByPrefix(spec, profilersPrefix)
	if err != nil {
		return nil, err
	}
	if len(profilersInfo) == 0 {
		return nil, nil
	}

	if len(profilersInfo) > 1 {
		log.Warnf("multiple profilers found, using %q", profilersInfo[0])
	}

	parts := strings.Split(profilersInfo[0], "___")
	if len(parts) != 4 {
		return nil, fmt.Errorf("invalid profiler info: %q", profilersInfo[0])
	}
