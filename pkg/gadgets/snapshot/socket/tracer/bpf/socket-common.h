...
socket_bpf_seq_print(struct seq_file *seq, const char *protocol,
		     const __be32 src, const __u16 srcp, const __be32 dest,
		     const __u16 destp, const unsigned char state, long ino)
{
	...
	BPF_SEQ_PRINTF(seq, "%s %08X %04X %08X %04X %02X %lu\n", protocol,
		       bpf_ntohl(src), bpf_ntohs(srcp), bpf_ntohl(dest),
		       bpf_ntohs(destp), state, ino);
}
