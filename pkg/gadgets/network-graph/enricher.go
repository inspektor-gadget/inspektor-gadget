...
func (e *Enricher) convertEdge(edge nettracer.Edge, pods *corev1.PodList, svcs *corev1.ServiceList) types.Edge {
	...
	if pods.Items[localPodIndex].OwnerReferences != nil {
		nameItems := strings.Split(out.LocalPodName, "-")
		if len(nameItems) > 2 {
			out.LocalPodOwner = strings.Join(nameItems[:len(nameItems)-2], "-")
		}
	}
...
func (e *Enricher) Enrich(edges []nettracer.Edge) (out []types.Edge) {
	var err error
	pods := &corev1.PodList{}
	...
	for _, edge := range edges {
		out = append(out, e.convertEdge(edge, pods, svcs))
	}
	return out
}
