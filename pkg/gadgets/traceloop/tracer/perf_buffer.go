...
func readOverWritable(reader *reader, mntnsID uint64, callback func(record perf.Record, size uint32) error) error {
	// Get private field with reflect.
	v := reflect.ValueOf(reader.reader)
	rings := reflect.Indirect(v).FieldByName("rings")
	ringsNumber := rings.Len()

	// For each perf buffer, we have as many buffers than CPU.
	for i := 0; i < ringsNumber; i++ {
		rr := reflect.Indirect(rings.Index(i)).FieldByName("ringReader")
