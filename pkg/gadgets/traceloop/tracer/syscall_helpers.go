...
func gatherSyscalls() (map[string]syscallDeclaration, error) {
...
func syscallGetParametersNumber(syscalls map[string]syscallDeclaration, syscallName string) (uint8, error) {
	syscall, ok := syscalls[syscallName]
	if !ok {
		return 0, fmt.Errorf("no syscall correspond to %q", syscallName)
	}

	return uint8(len(syscall.params)), nil
}

func syscallGetParameterName(syscalls map[string]syscallDeclaration, syscallName string, paramNumber uint8) (string, error) {
	syscall, ok := syscalls[syscallName]
	if !ok {
		return "", fmt.Errorf("no syscall correspond to %q", syscallName)
	}

	if int(paramNumber) >= len(syscall.params) {
		return "", fmt.Errorf("param number %d out of bound for syscall %q", paramNumber, syscallName)
	}

	return syscall.params[paramNumber].name, nil
}
