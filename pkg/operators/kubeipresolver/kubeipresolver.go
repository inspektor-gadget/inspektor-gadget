			...
			for _, ep := range endpoints {
				...
				k8sKindAcc, err := k8sSubAcc.AddSubField("kind", api.Kind_String,
					datasource.WithAnnotations(map[string]string{
						datasource.ColumnsWidthAnnotation: "12",
					}),
	...
	for ds, acc := range m.endpointsAccessors {
		ds.Subscribe(func(source datasource.DataSource, data datasource.Data) error {
			...
			for _, a := range acc {
				...
				if len(ips) != 1 {
					errors.Join(errs, fmt.Errorf("%s: expected %d %q field, got %d", a.root.Name(), 1, ipAddrType, len(ips)))
					continue
				}
				if ips[0].Size() != 16 {
					errors.Join(errs, fmt.Errorf("%s: expected %q field to have size %d, got %d", a.root.Name(), ipAddrType, 16, ips[0].Size()))
					continue
				}

				version := a.root.GetSubFieldsWithTag("name:version")
				if len(version) != 1 {
					errors.Join(errs, fmt.Errorf("%s: expected %d %q field, got %d", a.root.Name(), 1, "version", len(version)))
					continue
				}

				addrStr, err := common.GetIPForVersion(data, version[0], ips[0])
				if err != nil {
					errors.Join(errs, fmt.Errorf("%s: failed to get IP: %w", a.root.Name(), err))
					continue
				}
