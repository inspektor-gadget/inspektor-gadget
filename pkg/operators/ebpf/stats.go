...
func enrichStat(stat *stat, processMap map[uint32][]processmap.Process) {
	if procs, ok := processMap[stat.programID]; ok {
...
func (i *ebpfOperatorDataInstance) getStats() ([]stat, error) {
	...
	processMap, err := i.processMap.Get()
	if err != nil {
...
func (i *ebpfOperatorDataInstance) Stop(gadgetCtx operators.GadgetContext) error {
	...
	if err := bpfstats.DisableBPFStats(); err != nil {
		errs = append(errs, fmt.Errorf("disabling bpf stats: %w", err))
	}
