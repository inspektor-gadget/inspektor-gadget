...
type (
	btfTypeValidator func(btf.Type, string) (bool, error)
...
func (i *ebpfInstance) validateGlobalConstVoidPtrVar(t btf.Type, varName string) (bool, error) {
	btfVar, ok := t.(*btf.Var)
	if !ok {
		return false, nil
	}
	...
	if btfVar.Linkage != btf.GlobalVar {
		return false, fmt.Errorf("%q is not a global variable", btfVar.Name)
	}

	btfPtr, ok := btfVar.Type.(*btf.Pointer)
	if !ok {
		return false, fmt.Errorf("%q is not a pointer", btfVar.Name)
	}

	btfConst, ok := btfPtr.Target.(*btf.Const)
	if !ok {
		return false, fmt.Errorf("%q is not const", btfVar.Name)
	}

	_, ok = btfConst.Type.(*btf.Void)
	if !ok {
		return false, fmt.Errorf("%q is not a const void pointer", btfVar.Name)
	}
