...
func (i *ebpfInstance) attachProgram(gadgetCtx operators.GadgetContext, p *ebpf.ProgramSpec, prog *ebpf.Program) (link.Link, error) {
	...
	attachToAnnotation := i.config.GetString("programs." + p.Name + ".annotations.attach_to")
	if attachToAnnotation != "" {
		i.logger.Debugf("Overriding attachTo with annotation %q for program %q", attachToAnnotation, p.Name)
		attachTo = attachToAnnotation
	}
