...
func (i *ebpfInstance) verifyPolicy() error {
	// Load the operator configuration from viper
	...
	// Verify the collection spec against the policy
	if err := verifyCollectionSpec(i.collectionSpec, policy); err != nil {
		return err
	}
...
var programTypeNameToID = map[string]ebpf.ProgramType{
	...
	"unspecified":             ebpf.UnspecifiedProgram,
...
// Built from helperNameToID for reverse lookups.
var helperIDToName map[asm.BuiltinFunc]string

// programTypeIDToName maps BPF program type IDs to their names.
// Built from programTypeNameToID for reverse lookups.
var programTypeIDToName map[ebpf.ProgramType]string

func init() {
	helperIDToName = make(map[asm.BuiltinFunc]string, len(helperNameToID))
	for name, id := range helperNameToID {
		helperIDToName[id] = name
	}

	programTypeIDToName = make(map[ebpf.ProgramType]string, len(programTypeNameToID))
...
func helperNameToFunc(s string) (asm.BuiltinFunc, error) {
	...
	if s == "" {
		return 0, fmt.Errorf("empty helper name")
	}
	...
	// Require bpf_ prefix
	if !strings.HasPrefix(normalized, "bpf_") {
		return 0, fmt.Errorf("invalid BPF helper name %q: must start with 'bpf_'", s)
	}

	// Look up with the full name (including bpf_ prefix)
	if id, ok := helperNameToID[normalized]; ok {
		return id, nil
	}
...
func helperFuncToName(h asm.BuiltinFunc) string {
	if name, ok := helperIDToName[h]; ok {
		return name
	}
...
func programTypeToName(pt ebpf.ProgramType) string {
	if name, ok := programTypeIDToName[pt]; ok {
		return name
	}
	return fmt.Sprintf("unknown_prog_type_%d", pt)
}

// defaultHelpers is the default set of BPF helpers that are allowed.
// These are commonly used helpers for tracing and monitoring that are
// considered safe for observability use cases.
//
// Users can:
// - Add more helpers using the "add" list
// - Remove helpers using the "drop" list
var defaultHelpers = map[asm.BuiltinFunc]struct{}{
	// Map operations
	asm.FnMapLookupElem:       {},
	asm.FnMapUpdateElem:       {},
	asm.FnMapDeleteElem:       {},
...
// defaultProgramTypes is the default set of BPF program types that are allowed.
// These are typically used for tracing and monitoring purposes.
var defaultProgramTypes = map[ebpf.ProgramType]struct{}{
	// Kprobes and related
...
// validateAllKeyword checks that if "all" is present in a list, it's the only item.
// Returns true if "all" is present, false otherwise.
func validateAllKeyword(list []string, listName string) (bool, error) {
	hasAll := false
	for _, item := range list {
		if strings.TrimSpace(strings.ToLower(item)) == "all" {
			hasAll = true
			break
		}
	}

	if hasAll && len(list) > 1 {
		return false, fmt.Errorf("'all' keyword in %s cannot be used together with other items", listName)
	}
...
// It starts with the default lists, adds items from "add", and removes items from "drop".
// Special keyword "all":
//   - In "add" list: allows all helpers/program types (adds all from helperNameToID/programTypeNameToID)
//   - In "drop" list: drops all default helpers/program types (clears the default set)
//   - "all" cannot be used together with other items in the same list
func NewPolicy(c *Config) (*policy, error) {
	// Validate that "all" is not used with other items in the same list
	addAllHelpers, err := validateAllKeyword(c.Policy.Helpers.Add, "helpers.add")
	if err != nil {
		return nil, err
	}
	dropAllHelpers, err := validateAllKeyword(c.Policy.Helpers.Drop, "helpers.drop")
	if err != nil {
		return nil, err
	}
	if addAllHelpers && dropAllHelpers {
		return nil, fmt.Errorf("'all' keyword cannot be used in both helpers.add and helpers.drop")
	}

	addAllProgramTypes, err := validateAllKeyword(c.Policy.ProgramTypes.Add, "programTypes.add")
	if err != nil {
		return nil, err
	}
	dropAllProgramTypes, err := validateAllKeyword(c.Policy.ProgramTypes.Drop, "programTypes.drop")
	if err != nil {
		return nil, err
	}
	if addAllProgramTypes && dropAllProgramTypes {
		return nil, fmt.Errorf("'all' keyword cannot be used in both programTypes.add and programTypes.drop")
	}

	// Start with defaults (copy the maps)
	p := &policy{
		helpers:      make(map[asm.BuiltinFunc]struct{}, len(defaultHelpers)),
		programTypes: make(map[ebpf.ProgramType]struct{}, len(defaultProgramTypes)),
	}

	// Add helpers
	if addAllHelpers {
		// Add all known helpers
		for _, helperID := range helperNameToID {
			p.helpers[helperID] = struct{}{}
		}
	} else {
		if !dropAllHelpers {
			for h := range defaultHelpers {
				p.helpers[h] = struct{}{}
			}
		}
	...
	// Add program types
	if addAllProgramTypes {
		// Add all known program types
		for _, progTypeID := range programTypeNameToID {
			p.programTypes[progTypeID] = struct{}{}
		}
	} else {
		if !dropAllProgramTypes {
			for pt := range defaultProgramTypes {
				p.programTypes[pt] = struct{}{}
			}
		}
