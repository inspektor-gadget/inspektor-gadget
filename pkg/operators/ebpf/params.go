...
func handleWellKnownParam(btfVar *btf.Var, p *param) {
	...
	for _, t := range types {
		switch {
		case t == ebpftypes.PidTypeName && btfVar.Name == "targ_pid":
			p.Key = "pid"
			p.Description = "Show only events generated by processes with this pid"
			return
		case t == ebpftypes.TidTypeName && btfVar.Name == "targ_tid":
			p.Key = "tid"
			p.Description = "Show only events generated by threads with this tid"
			return
		case t == ebpftypes.UidTypeName && btfVar.Name == "targ_uid":
			p.Key = "uid"
			p.Description = "Show only events generated by processes with this uid"
			return
		case t == ebpftypes.GidTypeName && btfVar.Name == "targ_gid":
			p.Key = "gid"
			p.Description = "Show only events generated by processes with this gid"
			return
		case t == ebpftypes.CommTypeName && btfVar.Name == "targ_comm":
			p.Key = "comm"
			p.Description = "Show only events generated by processes with this name"
			return
		}
	}
}
