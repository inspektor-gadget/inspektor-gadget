...
func (i *ebpfInstance) Init(gadgetCtx operators.GadgetContext) (api.Params, error) {
	...
	if err := i.register(gadgetCtx); err != nil {
		return nil, fmt.Errorf("registering datasources: %w", err)
	}

	if err := i.initFormatters(gadgetCtx); err != nil {
		return nil, fmt.Errorf("initializing formatters: %w", err)
	}
	...
	for _, p := range i.collectionSpec.Programs {
		...
		case ebpf.SchedCLS:
			...
			default:
				return nil, fmt.Errorf("unsupported hook type %q", parts[1])
			}

			handler, err := tchandler.NewHandler(direction)
			if err != nil {
				return nil, fmt.Errorf("creating tc network tracer: %w", err)
			}
