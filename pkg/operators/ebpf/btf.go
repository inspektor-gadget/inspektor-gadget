...
func configurableArrayParamName(name string) string {
...
func (i *ebpfInstance) fixBTFStructs(typesCpy *btf.Spec) (*btf.Spec, error) {
	...
	for typ, err := range typesCpy.All() {
		...
		for _, member := range eventStructure.Members {
			...
			if !slices.Contains(typeNames, ebpfTypes.ConfigurableArrayTypeName) {
				continue
			}
			...
			fieldSizeStr, ok := i.paramValues[configurableArrayParamName(member.Name)]
			if !ok {
				continue
			}
...
func (i *ebpfInstance) populateVariableSizeParams(typ btf.Type, name string) error {
	...
	for _, member := range typStruct.Members {
		...
		if !slices.Contains(typeNames, types.ConfigurableArrayTypeName) {
			continue
		}
		...
		paramName := configurableArrayParamName(member.Name)
