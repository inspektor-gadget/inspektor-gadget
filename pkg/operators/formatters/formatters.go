...
type replacer struct {
	...
	// replacer will be called for incoming data with the source and target fields set
	replacer func(datasource.DataSource, datasource.FieldAccessor) (func(datasource.Data) error, error)
...
var replacers = []replacer{
	{
		...
		replacer: func(ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
			...
			return func(data datasource.Data) error {
				...
				default:
					return nil
				case 8:
					// TODO: this is only time since boot
					t := time.Unix(0, int64(binary.LittleEndian.Uint64(inBytes)))
	...
	},
	{
		...
		replacer: func(ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
			...
			return func(entry datasource.Data) error {
				...
				switch v[0] {
				case 4:
					out.Set(entry, []byte(net.IP(ip[:4]).String()))
				case 6:
					out.Set(entry, []byte(net.IP(ip).String()))
				}
				return nil
			}, nil
		},
...
func (f *formattersOperatorInstance) ParamDescs() params.ParamDescs {
	return nil
}

func (f *formattersOperatorInstance) init(ctx operators.GadgetContext) {
	// Find things we can enrich
	for _, ds := range ctx.GetDataSources() {
		var converters []converter
		ctx.Logger().Debugf("inspecting datasource %q", ds.Name())
		for _, r := range replacers {
			fields := ds.GetFieldsWithTag(r.selectors...)
			if len(fields) == 0 {
				continue
			}
			ctx.Logger().Debugf("> found %d fields for replacer %v", len(fields), r.selectors)
			for _, field := range fields {
				replFunc, err := r.replacer(ds, field)
				if err != nil {
					ctx.Logger().Debugf(">  skipping field %q: %v", field.Name(), err)
					continue
				}
				if replFunc == nil {
					continue
...
func (f *formattersOperatorInstance) Prepare(ctx operators.GadgetContext) error {
...
func (f *formattersOperatorInstance) Start(ctx operators.GadgetContext) error {
	return nil
}
