...
func handleL3Endpoint(in datasource.FieldAccessor) (func(entry datasource.Data) (string, error), error) {
	...
	ips := in.GetSubFieldsWithTag("type:" + types.IPAddrTypeName)
	if len(ips) != 1 {
		return nil, fmt.Errorf("expected %d %q field, got %d", 1, types.IPAddrTypeName, len(ips))
	}
	if ips[0].Size() != 16 {
		return nil, fmt.Errorf("expected %q to have 16 bytes, got %d", types.IPAddrTypeName, ips[0].Size())
	}
...
var replacers = []replacer{
	{
		name:      "signal",
		selectors: []string{"type:" + types.SignalTypeName},
	...
	{
		name:      "errno",
		selectors: []string{"type:" + types.ErrnoTypeName},
		replace: func(logger logger.Logger, ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
	...
	{
		name:      "syscall",
		selectors: []string{"type:" + types.SyscallTypeName},
		replace: func(logger logger.Logger, ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
	...
	{
		name:      "timestamp",
		selectors: []string{"type:" + types.TimestampTypeName},
		replace: func(logger logger.Logger, ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
	...
	{
		name:      "l3endpoint",
		selectors: []string{"type:" + types.L3EndpointTypeName},
		replace: func(logger logger.Logger, ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
	...
	{
		name:      "l4endpoint",
		selectors: []string{"type:" + types.L4EndpointTypeName},
		replace: func(logger logger.Logger, ds datasource.DataSource, in datasource.FieldAccessor) (func(data datasource.Data) error, error) {
