...
func GetTotalMemory() (uint64, error) {
	...
	for scanner.Scan() {
		...
		if !bytes.HasPrefix(fields[0], []byte("MemTotal")) {
			continue
		}
		memBytes, ok := bytes.CutSuffix(bytes.TrimSpace(fields[1]), []byte(" kB"))
		if !ok {
			return 0, fmt.Errorf("unexpected contents of total memory field: %q", string(fields[1]))
		}
	...
	if needStatus(options) {
		...
		for {
			...
			if e != nil {
				s.Close()
				bufPool.Put(xbuf)
				return pi, fmt.Errorf("read status %d: %w", pid, e)
			}
		}
		s.Close()
		data := (*xbuf)[:n]
	...
	if needStat(options) {
		...
		for {
			...
			if e != nil {
				s.Close()
				bufPool.Put(xbuf)
				return pi, fmt.Errorf("read stat %d: %w", pid, e)
			}
		}
		s.Close()
		data := (*xbuf)[:n]
