...
func GetContainerSelectorParams(op string) params.ParamDescs {
	commonParams := params.ParamDescs{
		{
			Key:            ParamPodName,
	...
	if op == "KubeManager" {
		runtimeContainerParam.Key = ParamRuntimeContainerName
		// containername and k8s-containername aliases of each other
		...
		k8sContainerNameParam.Key = ParamK8sContainerName
		// containername and runtime-containername aliases of each other
	...
	return append(commonParams, &k8sContainerNameParam, &runtimeContainerParam)
}
