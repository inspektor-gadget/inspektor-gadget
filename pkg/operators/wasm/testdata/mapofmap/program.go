...
func gadgetStart() int {
	...
	if err != nil {
		api.Errorf("setting %s FD value for key %v in %s: %w", mapName, key, mapOfMapName, err)
		return 1
	}
	...
	if err == nil {
		api.Errorf("should not be able setting %s FD value for key %v in %s: %w", mapName, key, mapOfMapName, err)
		return 1
	}
