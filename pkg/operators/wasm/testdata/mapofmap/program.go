...
func gadgetStart() int {
	...
	err = mapOfMap.Update(key, hashMap, api.UseFdFlag)
	if err != nil {
	...
	if err == nil {
		api.Errorf("updating %s, which type is BPF_MAP_TYPE_HASH_OF_MAPS, without UseFdFlag", mapOfMapName)
		return 1
	}
	...
	err = mapOfMap.Update(key, arrayMap, api.UseFdFlag)
	if err == nil {
