...
func (i *wasmOperatorInstance) kallsymsSymbolExists(ctx context.Context, m wapi.Module, stack []uint64) {
	...
	if i.kernelSymbolResolver == nil {
		var err error
		i.kernelSymbolResolver, err = kallsyms.NewKAllSyms()
		if err != nil {
			i.logger.Warnf("kallsymsSymbolExists: initializing kernel symbol resolver: %v", err)
			stack[0] = 0
			return
		}
	}
	...
	ret := uint32(0)
	if i.kernelSymbolResolver.SymbolExists(symbolName) {
		ret = 1
	}
