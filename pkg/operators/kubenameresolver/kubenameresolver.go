...
func (m *KubeNameResolverInstance) enrichSingle(data datasource.Data, accessor k8sAccesors) error {
	...
	// generate the network policy name.
	if pod.OwnerReferences != nil {
		// Trim the pod name to remove the suffix after the second to last dash
		// (e.g. "pod-name-1234567890-abcde" -> "pod-name")
		numDashes := 0
		lastIndex := len(pod.Name) - 1
		for ; lastIndex >= 0; lastIndex-- {
			if pod.Name[lastIndex] != '-' {
				continue
			}
		...
		if numDashes == 2 {
			accessor.Owner.PutString(data, pod.Name[:lastIndex])
		}
	}
