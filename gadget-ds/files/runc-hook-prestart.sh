...
if [ ! -f $BPFDIR/cgrouplabelsmap ] ; then
  ...
  $BPFTOOL map create $INNERMAPMETA type array key 4 value 64 entries 64 name containermapinnermeta
  ...
  # create map from cgroup ID to label map, filled with maps for all pods here later, read by *snoop-edge
  ...
  $BPFTOOL map create $BPFDIR/cgroupmetadatamap type hash_of_maps innermap pinned $INNERMAPMETA key 8 value 4 entries 8000 name cgroupmetadatamap flags 1
fi
...
$KUBECTL --kubeconfig=/etc/kubernetes/kubeconfig get pod --all-namespaces -o json | tee /tmp/kubectl-get-pods-${CGROUP_ID}.json | \
  ...
  while IFS=$'\t' read -r uid namespace podname nodename labels containername; do
    ...
    echo "  $podname_hex"
    $BPFTOOL map update pinned $BPFDIR/metadata$CGROUP_ID key 0 value hex $namespace_hex
    echo "bpftool map updated returned: $?"
    $BPFTOOL map update pinned $BPFDIR/metadata$CGROUP_ID key 1 value hex $podname_hex
    echo "bpftool map updated returned: $?"
    # update cgroup ID map to point to this new pod map
    $BPFTOOL map update pinned $BPFDIR/cgroupmetadatamap key hex $CGROUP_ID_HEX value pinned $BPFDIR/metadata$CGROUP_ID
    echo "bpftool map updated returned: $?"
