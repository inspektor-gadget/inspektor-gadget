...
RUNC_HOOKS_LDPRELOAD=0
if [[ "$INSPEKTOR_GADGET_OPTIONS" =~ (^|,)runc_hooks_ldpreload(,|$) ]] ; then
  RUNC_HOOKS_LDPRELOAD=1
fi

if [[ "$RUNC_HOOKS_LDPRELOAD" = 1 ]] ; then
...
if [ "$FLATCAR_EDGE" = 1 ] || [ "$RUNC_HOOKS_LDPRELOAD" = 1 ] ; then
...
if [[ ! "$INSPEKTOR_GADGET_OPTIONS" =~ (^|,)notraceloop(,|$) ]] ; then
