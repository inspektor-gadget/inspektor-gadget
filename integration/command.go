...
func WaitUntilPodReadyCommand(namespace string, podname string) *Command {
	return &Command{
		...
		Cmd:            fmt.Sprintf("kubectl wait pod --for condition=created --timeout=10s -n %s %s || kubectl wait pod --for condition=ready --timeout=60s -n %s %s", namespace, podname, namespace, podname),
		ExpectedRegexp: fmt.Sprintf("pod/%s condition met\n", podname),
	}
}
