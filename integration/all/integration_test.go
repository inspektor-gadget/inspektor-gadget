...
func testMainIG(m *testing.M) int {
	fmt.Println("Start running tests:")
	return m.Run()
}
...
func TestMain(m *testing.M) {
	...
	case "inspektor-gadget":
		if os.Getenv("KUBECTL_GADGET") == "" {
			fmt.Println("$KUBECTL_GADGET not set")
			os.Exit(1)
		}
	...
	default:
		fmt.Printf("invalid argument '-test-component': %q. Valid values: ig, inspektor-gadget", *testComponent)
		os.Exit(1)
	}
}
