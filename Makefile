...
local-gadget-%: phony_explicit
	...
	docker build -t local-gadget-$*-builder -f $(DOCKERFILES_DIR)/local-gadget.Dockerfile \
...
gadget-default-container:
	docker buildx build -t $(CONTAINER_REPO):$(IMAGE_TAG) -f $(DOCKERFILES_DIR)/gadget-default.Dockerfile \
		--build-arg ENABLE_BTFGEN=$(ENABLE_BTFGEN) .

# gadget CO-RE container image
.PHONY: gadget-core-container
gadget-core-container:
	docker buildx build -t $(CONTAINER_REPO):$(IMAGE_TAG) -f $(DOCKERFILES_DIR)/gadget-core.Dockerfile \
		--build-arg ENABLE_BTFGEN=$(ENABLE_BTFGEN) .
...
kubectl-gadget-container:
	docker build -t kubectl-gadget -f $(DOCKERFILES_DIR)/kubectl-gadget.Dockerfile \
