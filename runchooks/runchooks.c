		...
		"{"
			"\"hooks\": {"
				"\"prestart\": ["
					...
					"{"
						"\"path\": \"/bin/sh\","
						"\"args\": [\"sh\", \"-c\", \"test ! -x /opt/bin/ocihookotel || /opt/bin/ocihookotel -hook prestart\"]"
					"}"
				"],"
				"\"poststop\": ["
					"{"
					...
					"{"
						"\"path\": \"/bin/sh\","
						"\"args\": [\"sh\", \"-c\", \"test ! -x /opt/bin/ocihookotel || /opt/bin/ocihookotel -hook poststop\"]"
					"}"
				"]"
			"}"
		"}";

	ret = snprintf(cmd, sizeof(cmd), "cd %s && cp config.json config.json.orig && cat config.json.orig | jq -r '. * %s' > config.json", bundledir, patch);
	free(bundledir);
	if (ret < sizeof(cmd))
		system(cmd);
}
