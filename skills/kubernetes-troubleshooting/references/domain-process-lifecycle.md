...
- **`top_process`** — periodic process stats (CPU/mem ranking) — "what's hot
  ...
  e.g. `--sort -memRSS_raw` for the biggest memory users), `--interval` (default
...
kubectl gadget run top_process:latest -n <ns> --sort -memRSS_raw --interval 5 --max-entries 10 -o json
```
