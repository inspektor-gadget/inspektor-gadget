...
#define GADGET_TRACER(name, map_name, event_type)                                           \
	_Pragma("GCC warning \"GADGET_TRACER is deprecated. Used BTF gadget declaration\"") \
...
#define GADGET_PARAM(name) \
	const void *gadget_param_##name __attribute__((unused)) SEC(".params");
...
#define GADGET_SNAPSHOTTER(name, type, ...)                                                      \
	_Pragma("GCC warning \"GADGET_SNAPSHOTTER is deprecated. Used BTF gadget declaration\"") \
...
#define GADGET_MAPITER(name, mapname)                                                        \
	_Pragma("GCC warning \"GADGET_MAPITER is deprecated. Used BTF gadget declaration\"") \
		const void *gadget_mapiter_##name##___##mapname                              \
		__attribute__((unused));

// GADGET_GEN_TYPE_BTF ensures the compiler generates BTF information for the given type.
#define GADGET_GEN_TYPE_BTF(type)                        \
	const type *__CONCAT(__unused_var_, __COUNTER__) \
		__attribute__((unused));
