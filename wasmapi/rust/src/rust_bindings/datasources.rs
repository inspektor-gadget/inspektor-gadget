...
use crate::{
    errorf,
    rust_bindings::{helpers::string_to_buf_ptr, log::LogLevel},
}; //relative paths may hinder in testing.
...
impl DataSource {
    ...
    pub fn subscribe_data(&self, cb: DataFunc, prio: u32) -> Result<()> {
        self.subscribe(SubscriptionType::Data, prio, CallBack::Data(cb))
    }

    pub fn subscribe_array(&self, cb: ArrayFunc, prio: u32) -> Result<()> {
        self.subscribe(SubscriptionType::Array, prio, CallBack::Array(cb))
    }

    pub fn subscribe_packet(&self, cb: PacketFunc, prio: u32) -> Result<()> {
        self.subscribe(SubscriptionType::Packet, prio, CallBack::Packet(cb))
    }
...
pub fn dataSourceCallback(cb_id: u64, ds: u32, data: u32) {
    ...
    match sub.typ {
        SubscriptionType::Data => {
            if let CallBack::Data(cb) = sub.cb {
                cb(DataSource(ds), Data(data));
            }
        }
        SubscriptionType::Array => {
            if let CallBack::Array(cb) = sub.cb {
                match cb(DataSource(ds), DataArray(data)) {
                    Ok(_) => {}
                    Err(err) => {
                        errorf!("Dataarray callback error: {:?}", err);
                    }
                };
            }
        }
        SubscriptionType::Packet => {
            if let CallBack::Packet(cb) = sub.cb {
                match cb(DataSource(ds), Packet(data)) {
                    Ok(_) => {}
                    Err(err) => {
                        errorf!("Dataarray callback error: {:?}", err);
                    }
