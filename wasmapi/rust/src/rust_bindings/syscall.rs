...
use crate::rust_bindings::helpers::{
    any_to_buf_ptr, bytes_to_buf_ptr, from_c_string, string_to_buf_ptr,
};

#[link(wasm_import_module = "ig")]
extern "C" {
    #[link_name = "getSyscallName"]
    fn syscall_name(id: u32, dst: u64) -> u32;
    #[link_name = "getSyscallID"]
    fn syscall_id(name: u64) -> i32;
    #[link_name = "getSyscallDeclaration"]
    fn syscall_declaration(name: u64, pointer: u64) -> u32;
}
...
pub fn get_syscall_name(id: u16) -> Result<String> {
    ...
    let result = unsafe { syscall_name(id as u32, bytes_to_buf_ptr(&dst) as u64) };
...
pub fn get_syscall_id(name: String) -> Result<i32> {
    let id = unsafe { syscall_id(string_to_buf_ptr(&name)) };
...
pub fn get_syscall_declaration(name: &str) -> Result<SyscallDeclaration> {
    let raw_decl = SyscallDeclarationRaw::default();
    let buf_ptr = any_to_buf_ptr(&raw_decl).expect("Invalid raw syscall declaration");
    let ret = unsafe { syscall_declaration(string_to_buf_ptr(name) as u64, buf_ptr) };
