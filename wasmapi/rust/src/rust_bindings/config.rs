...
use crate::rust_bindings::{datasources::FieldKind, helpers::string_to_buf_ptr};

#[link(wasm_import_module = "ig")]
extern "C" {
    #[link_name = "setConfig"]
    fn set_conf(key: u64, val: u64, kind: u32) -> u32;
}
pub trait Allowed {
...
pub fn set_config<T: Allowed>(key: String, val: &T) -> Result<(), String> {
    ...
    let ret = unsafe { set_conf(key_ptr, val_ptr, FieldKind::String as u32) };
    if ret != 0 {
        Err("setting config".into())
    } else {
        Ok(())
    }
}
