...
func (r *Runtime) runGadget(runner runtime.Runner, gadget gadgets.GadgetInstantiate, operatorPerGadgetParamCollection params.ParamsCollection, params params.Params) error {
	log := runner.Logger()
	...
	if setter, ok := gadgetInstance.(gadgets.EventHandlerSetter); ok {
		...
		switch gadget.Type() {
		default:
			setter.SetEventHandler(runner.Columns().EventHandlerFunc(runner.Operators().Enrich))
		}
	}
	...
	if setter, ok := gadgetInstance.(gadgets.EventHandlerArraySetter); ok {
		...
		switch gadget.Type() {
		default:
			setter.SetEventHandlerArray(runner.Columns().EventHandlerFuncArray(runner.Operators().Enrich))
		case gadgets.TypeTraceIntervals:
			setter.SetEventHandlerArray(runner.Columns().EventHandlerFuncSnapshot("main", runner.Operators().Enrich)) // TODO: "main" is the node
		}
	}
