...
func NewRunCommand(rootCmd *cobra.Command, runtime runtime.Runtime, hiddenColumnTags []string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PreRunE: func(cmd *cobra.Command, args []string) error {
			// Manually register the clioperator
			clioperator.Register()
			...
			info, err := runtime.GetOCIGadgetInfo(gadgetCtx)
			if err != nil {
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			gadgetCtx := gadgetcontext.NewOCI(ctx, args[0], logger.DefaultLogger(), params.Collection{
				"oci": ociParams,
			}) // TODO
