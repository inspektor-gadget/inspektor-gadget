...
func AddCommandsFromRegistry(rootCmd *cobra.Command, runtime runtime.Runtime, columnFilters []cols.ColumnFilter) {
	...
	for _, gadgetDesc := range gadgetregistry.GetGadgetDescs() {
		...
		cmd.AddCommand(buildCommandFromGadget(gadgetDesc, columnFilters, runtime, runtimeParams, operatorsParamsCollection))
	}
}

func buildGadgetDoc(gadgetDesc gadgets.GadgetDesc, parser parser.Parser) string {
	var out strings.Builder
	fmt.Fprintf(&out, "%s\n\n", gadgetDesc.Description())

	if parser != nil {
		fmt.Fprint(&out, "Available columns:\n")
...
func buildCommandFromGadget(
	...
	cmd := &cobra.Command{
		...
		Long:         buildGadgetDoc(gadgetDesc, parser),
