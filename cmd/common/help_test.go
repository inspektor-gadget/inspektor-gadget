...
// TestRunCommandStillRequiresRootForExecution verifies that actual gadget execution
// still requires root permissions.
func TestRunCommandStillRequiresRootForExecution(t *testing.T) {
	// Skip if running as root, since we want to test the non-root behavior
	if os.Geteuid() == 0 {
		t.Skip("Test requires running as non-root user")
	}
	...
	// Test that actual gadget execution still requires root
	runCmd.SetArgs([]string{"ghcr.io/inspektor-gadget/gadget/trace_open:latest"})
	err := runCmd.Execute()
	require.Error(t, err, "Gadget execution should still require root permissions")
	require.Contains(t, err.Error(), "must be run as root", "Error should mention root requirement")
}
