...
type ByobFlags struct {
	OCIImage string
	File     string
}

func NewByobCmd(runCmd func(*cobra.Command, []string) error, flags *ByobFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "byob",
		Short: "Trace with your own BPF program",
		RunE:  runCmd,
	}
...
func LoadByobFile(path string) (string, error) {
