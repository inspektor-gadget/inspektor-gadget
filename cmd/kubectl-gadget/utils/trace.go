...
func createTraces(trace *gadgetv1alpha1.Trace) error {
	...
	if trace.Spec.Node != "" && !nodeFound {
		return fmt.Errorf("Invalid filter: Node %q does not exist", trace.Spec.Node)
	}
...
func labelsFromFilter(filter map[string]string) string {
	...
	for labelName, labelValue := range filter {
		...
		// If labels is not empty, we will need to separate the other label value
		// with a comma, so change separator value.
		if labels != "" {
			separator = ","
		}
