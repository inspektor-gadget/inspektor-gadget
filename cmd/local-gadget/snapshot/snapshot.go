...
func (g *SnapshotGadget[Event]) Run() error {
	...
	entries, err := g.runTracer(localGadgetManager, containerSelector)
	if err != nil {
		return commonutils.WrapInErrGadgetTracerCreateAndRun(err)
	}

	g.parser.SortEvents(&entries)
...
func NewSnapshotCmd() *cobra.Command {
	traceCmd := commonsnapshot.NewCommonSnapshotCmd()

	traceCmd.AddCommand(newProcessCmd())
