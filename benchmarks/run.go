...
const (
	NumRuns = 1 // Number of times to run each test configuration
)
...
type GadgetClientServerTest struct {
	Gadget      string
	ServerImage string
	ClientImage string
	Combs       []any

	ServerCmd func(a any) string
	ClientCmd func(serverIP string, a any) string
}

func testGadgetSingle(t *testing.T, c *GadgetClientServerTest, comb any, usetracer bool) stat {
	...
	clientContainerOpts := []containers.ContainerOption{containers.WithContainerImage(c.ClientImage)}
	...
	clientCmd := c.ClientCmd(serverIP, comb)
...
func testGadgetMultiple(t *testing.T, c *GadgetClientServerTest, comb any, usetracer bool) statResult {
...
func RunGadgetTest(t *testing.T, c *GadgetClientServerTest) {
	t.Helper()
